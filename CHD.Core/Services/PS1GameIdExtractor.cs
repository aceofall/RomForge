...
public class PS1GameIdExtractor(LibChdrWrapper chd)
{
    ...
    public string? ExtractGameId()
    {
        ...
        if (pvd[dataOffset] != 0x01 || pvd[dataOffset + 1] != 'C' ||
            pvd[dataOffset + 2] != 'D' || pvd[dataOffset + 3] != '0' ||
            pvd[dataOffset + 4] != '0' || pvd[dataOffset + 5] != '1')
            return null;
    ...
    private string? ReadSystemCnf(uint lba, uint fileOffset, int dataOffset)
    {
        ...
        var boot = Regex.Match(content, @"BOOT\s*=\s*cdrom[:\\\/]+([A-Z]{4}_\d{3}\.\d+)",
                               RegexOptions.IgnoreCase);
        if (!boot.Success) return null;
