...
public static class NcchBuilder
{
    ...
    public static async Task BuildAsync(NcchUnpackResult unpack, byte[] exefsBlock, Stream ncchSource, RomFsUnpackResult? romFs, Stream output, IRomFsFileSource? patchSource = null, Action<long, long>? progress = null, CancellationToken ct = default)
    {
        ...
        if (romFs != null)
        {
            ...
            if (patchSource != null)
                patchSizeMap = await RomFsPacker.BuildPatchSizeMapAsync(romFs.Files, patchSource, ct);
    ...
    public static async Task<long> CalculateSizeAsync(NcchUnpackResult unpack, byte[] exefsBlock, RomFsUnpackResult? romFs, IRomFsFileSource? patchSource, CancellationToken ct = default)
    {
        long ncchSize = MediaUnit;
